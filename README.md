...
Pre-built debug APKs are also published under
[Releases](../../releases), signed with the debug key committed at
`keystore/debug.keystore` (generated once by CI, reused for every build so
installs upgrade cleanly instead of conflicting on signature).
